...
Флажь `low`, если:
- postconditions отсутствуют, но тест не меняет состояние системы (read-only сценарий). Добавь явное описание: `Состояние системы не изменено`.
...
Если postconditions отсутствуют и тест read-only: добавь `action = "Состояние системы не изменено"`, `expected = "Данные на экране совпадают с исходными"`.
